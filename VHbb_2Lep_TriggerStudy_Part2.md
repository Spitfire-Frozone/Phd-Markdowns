...
Since the numer of data evens in these bins are quite low, the last test that we want to do is to check how many data would pass the standard trigger.
...
So now lets get a fresh copy of the WSMaker
~~~
cd /afs/cern.ch/work/d/dspiteri/VHbb
setupATLAS && lsetup git 
git clone --recursive ssh://git@gitlab.cern.ch:7999/atlas-physics/higgs/hbb/WSMaker_VHbb.git
mv WSMaker_VHbb WSMaker_VHbb_METStudy
cd WSMaker_VHbb_METStudy
source setup.sh
mkdir inputs
cd build
cmake ..
...
However these are already the standars and should be present in WSMaker, so while you will need to split the inputs, you do not need to add a file to the config for the standard, but you will need one for the inputs you just generated. If you have the milestone 1 2019 October inputs, then you already have this and you don't need to split them either.
...
make -j10
cd ..
SplitInputs -r Run2 -v SMVHVZ_2019_MVA_mc16ade_v02_STXS
